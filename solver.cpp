...
void Solver<Dtype>::Step(int iters) {
  const int start_iter = iter_;// ??这里的iter_没有定义吧，而形参里也没有声明
  const int stop_iter = iter_ + iters;// 同样，iters也没有预先声明
  int average_loss = this->param_.average_loss();
  losses_.clear();
  smoothed_loss_ = 0;
  iteration_timer_.Start();

  while (iter_ < stop_iter) {// 是否大于最大的迭代次数
    // zero-init the params
    net_->ClearParamDiffs();// 消除参数梯度
    /*进行测试*/
...
void Solver<Dtype>::Solve(const char* resume_file) {
  ...
  if (requested_early_exit_) {// 需要提前退出优化训练
  ...
  if (param_.display() && iter_ % param_.display() == 0) {
    int average_loss = this->param_.average_loss();
    Dtype loss;
    net_->Forward(&loss);// only run a forward pass

    UpdateSmoothedLoss(loss, start_iter, average_loss);

    LOG(INFO) << "Iteration " << iter_ << ", loss = " << smoothed_loss_;// 显示当前train的loss
  }
